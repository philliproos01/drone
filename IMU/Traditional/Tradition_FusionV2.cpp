...
void loop() {
  ...
  // Calculate magnetometer-based yaw
  float mag_yaw = atan2(H_calibrated[1], H_calibrated[0]) * 180 / PI;

  // Ensure mag_yaw is in the range 0-360
  if (mag_yaw < 0) {
    mag_yaw += 360;
  }

  // Complementary filter for yaw
  yaw = 0.96 * (yaw + GyroZ * elapsedTime) + 0.04 * mag_yaw;
